...
class ShadowScorecard:
    ...
    def calculate_scorecard_metrics(self, df):
        ...
        # Early-day cohort badges
        early_day_sample = cohort_days < 5
        brier_display = f"{brier_improvement_pct:+.2f}%" if not early_day_sample else f"{brier_improvement_pct:+.2f}% (sample<5)"
        
        return {
            'total_days': len(df),
            'trading_days': len(df),  # Same for synthetic data
            'cohort_days': cohort_days,
            'early_day_sample': early_day_sample,
            'brier_improvement_pct': brier_improvement_pct,
            'brier_display': brier_display,
    ...
    def write_cohort_daily_log(self, metrics):
        """Write COHORT_DAILY.md log with day-by-day verdicts"""
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        audit_dir = Path('audit_exports') / 'daily' / timestamp
        audit_dir.mkdir(parents=True, exist_ok=True)
        
        cohort_log_file = audit_dir / 'COHORT_DAILY.md'
        
        # Get cohort progress
        cohort = self.get_cohort_day()
        
        content = f"""# Cohort Daily Log

**Generated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}
**Cohort ID**: SHADOW_30D_{cohort['start_date'].replace('-', '')}
**Progress**: Day {cohort['day']}/{cohort['total']}

## Daily Verdicts

| Day | Date | ΔBrier | ΔECE | ΔStraddle | Verdict |
|-----|------|--------|------|-----------|----------|
"""
        
        # In production, this would iterate through actual daily data
        # For now, show current day data
        today_date = datetime.now().strftime('%Y-%m-%d')
        verdict = "Collecting" if metrics['early_day_sample'] else "Good" if metrics['brier_improvement_pct'] > 0 else "Weak"
        
        content += f"| {cohort['day']}/30 | {today_date} | {metrics['brier_display']} | {metrics['ece_display']} | {metrics['straddle_improvement_pct']:+.2f}% | {verdict} |\n"
        
        content += f"""

## Current Status

- **Sample Size**: {'Small' if metrics['early_day_sample'] else 'Building'} (k={cohort['day']})
- **ECE Ready**: {'No' if metrics['ece_improvement_pct'] is None else 'Yes'} (needs ≥10 days)
- **Overall Trend**: {'Collecting data' if metrics['early_day_sample'] else 'Improving' if metrics['brier_improvement_pct'] > 1 else 'Stable'}

---
Generated by Cohort Daily Log v0.1
"""
        
        with open(cohort_log_file, 'w', encoding='utf-8') as f:
            f.write(content)
        
        return str(cohort_log_file)
...
### Performance Metrics
- **ΔBrier(cohort)**: {metrics['brier_display']} (candidate vs baseline)
...
"""
        ...
        # Write cohort daily log
        cohort_log_file = self.write_cohort_daily_log(metrics)
