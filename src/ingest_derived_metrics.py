# 📄 File: src/ingest_derived_metrics.py
#
# 📌 Title
# Zen Council – Stage 2.1 Derived Metrics Ingestion (Idempotent MERGE)
#
# 📝 Commit Notes
# Commit Title: ETL: add Stage 2.1 derived metrics ingestion (idempotent) and env hardening
# Commit Message:
# - Computes Daily Return, 10D rolling volatility, and ATR(14) from *_HISTORICAL tables.
# - Loads rows into a TEMP staging table, then MERGE into FORECAST_DERIVED_METRICS (no duplicates).
# - Adds .env loading + required env validation for Snowflake connection.
# - Safe to re-run; designed for CI job after daily ingestion.
# - Next: Stage 2.2 correlations as a separate script.
"""
Computes derived metrics and MERGEs them into FORECAST_DERIVED_METRICS.
"""
...
def build_rows(df: pd.DataFrame, symbol: str) -> List[Tuple]:
    ...
    return [
        (r.DATE.date(), symbol, float(r.DAILY_RETURN), float(r.VOLATILITY_10D), float(r.ATR_14D))
